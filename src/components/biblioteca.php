...
function LogOut(){
    ...
    function CadastrarUsuario($rm, $nome, $email, $senha, $telefone, $dt_nascimento, $genero){
        $sql = 'INSERT INTO usuario(rm, nome, email, senha, telefone, dt_nascimento, genero) VALUES ('.$rm.',"'.$nome.'","'.$email.'","'.$senha.'","'.$telefone.'","'.$dt_nascimento.'","'.$genero.'")';
        $res = $GLOBALS['conn']->query($sql);
        if($res){
  ...
  function CadastrarLivro($isbn, $titulo, $ano, $qtd, $sinopse, $classificacao, $id_editora, $id_genero, $id_autor, $estado, $capa){
    $sql = 'INSERT INTO livro(isbn, titulo, ano, qtd, sinopse, classificacao, id_editora, id_genero, id_autor,estado, capa) VALUES ("'.$isbn.'","'.$titulo.'","'.$ano.'","'.$qtd.'","'.$sinopse.'","'.$classificacao.'","'.$id_editora.'","'.$id_genero.'","'.$id_autor.'","'.$estado.'","'.$capa.'")';
    $res = $GLOBALS['conn']->query($sql);
    if($res){
...
function CadastrarEmprestimo($id_usuario, $id_livro, $dt_emprestimo, $dt_devolucao){
    $sqlCmd = 'INSERT INTO emprestimo(cd, dt_emprestimo, dt_devolucao, status, id_usuario, id_livro) VALUES (null,"'.$dt_emprestimo.'","'.$dt_devolucao.'","Emprestado","'.$id_usuario.'","'.$id_livro.'")';
    $res = $GLOBALS['conn']->query($sqlCmd);
    if($res){
        echo 'Empréstimo Cadastrado';
    }else{
        echo 'Erro ao Cadastrar Empréstimo: '.$GLOBALS['conn']->error;
    }
}
